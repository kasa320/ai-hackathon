...
| 自由文は下書き止まり | Discord・Webの自由文から読み取った値は下書き。本人が確認して送信したときだけ保存する。原文は保存せず、計画を作るAIにも渡さない。聞き直しの質問文は AI が書くが、本人が画面で見られる以上の情報は AI に渡さない |
...
| 参加者（member） | 参加条件の回答（参加するか・参加できる時間帯）。案への同意。割り振られた説明の担当を引き受けるかの回答 |
| 担当者 | エージェントが説明を割り振った参加者。引き受けるか本人が回答する。辞退もできる |
...
```mermaid
flowchart TD
    A[管理者が開催回を登録<br/>期間・1回の長さ・範囲] --> B[全員に参加できる日時を聞く]
    B --> C{全員の回答が<br/>そろった？}
    C -- まだ --> B
    C -- 期限切れ --> Z[管理者の判断待ち]
...
輪読は日程を決めてから範囲を読んでくるので、準備状況（読んできた範囲・説明できる範囲）は聞きません。

| 項目 | 値 | 日時未定の回 | 日時が決まっている回 |
| --- | --- | --- | --- |
| 参加できるか | 参加／欠席 | 聞く（「参加を希望」＝日時はこれから調整） | 聞く |
| 参加できる時間帯 | 下の表 | **聞く**（欠席なら聞かない） | 聞かない |
| 出られない日 | 日付 | 本人が言ったときだけ反映する | 聞かない |
| 1回に参加できる最大時間 | 分 | 本人が言ったときだけ反映する | 聞かない |
| 説明の担当の辞退 | はい | 本人が言ったとき・「担当を辞退する」を押したときだけ | 同じ |

時間帯を答えた人は参加とみなし、「参加しますか」を別に聞き直しません。欠席・辞退の理由は聞かず、記録もしません。

### 5.2 参加できる時間帯の形

| 状態 | 意味 | 時間帯 |
| --- | --- | --- |
| `provided` | 時間帯を答えた | 1件以上 |
| `unknown` | まだ分からない | なし |
| `unavailable` | 期間中は参加できない | なし |

- 時間帯は「毎週◯曜 HH:mm〜HH:mm」と「YYYY-MM-DD HH:mm〜HH:mm」の2種類。どちらも日本時間で、同じ日のうちに収める（終了の 24:00 は画面では 0:00 と表示）
- 「平日の夜」「土曜の午後」のような言い方は、AI が具体的な時間帯に直す（目安：夜=19:00-22:00 など）。保存前に本人が確認・修正できる
- 最大参加時間（`max_duration_minutes`）は 0＝指定なし。本人が言ったときだけ入る
- 各60件まで
...
sequenceDiagram
    ...
    alt フォーム・ボタンで入力
        M->>W: 項目を選んで送信
    else 自由文で入力（Discord の会話）
        W-->>M: 「いつ頃なら参加できそうですか？」
        M->>W: 「水曜の夜なら大丈夫です」
        W->>X: 原文（保存しない）
        X-->>W: 決められた項目の値と、足りない項目を聞く質問文（下書き）
        W-->>M: 質問文を整えて聞き直す（改行・URL・メンションは落とす）
        M->>W: 答える → 全部そろったら確認ボタンを押す
    end
    W->>App: 構造化した参加条件
...
| Discord DM | 「参加条件」と送ると「いつ頃なら参加できそうですか？」と聞かれる。ふだんの言葉で答えれば AI が値を取り出し、足りない項目は AI が会話調で聞き直す。すべてそろうと確認ボタンが出る。押したときだけ保存される |
...
1. 会の**全員**について、その日に参加できる時間帯を求める
   ...
   - 本人が言った最大参加時間が会の長さより短い人がいれば、候補は0件（言っていなければ制限なし）
...
C が「45分までなら」と言っていた場合は、会の長さ60分に届かないため候補は0件になります。
...
準備状況は聞かないので、担当はエージェントが割り振り、割り振られた本人が引き受けるかを答えます。

| 順 | ルール |
| --- | --- |
| 1 | 担当候補は参加予定で、担当を辞退しておらず、この案件で引き受けを断っていない人 |
| 2 | 今回の範囲を順番どおりのまとまりに分け、まとまりごとに1人を割り振る（なるべく多くの人に分ける。1人5分以上取れる人数まで） |
| 3 | 今の確定計画の担当者で、まだ候補にいる人は、同じまとまりを続ける |
| 4 | 残りのまとまりは、**これまで担当した回数が少ない人 → 続けて代役になった回数が少ない人 → 登録順**で割り振る |
| 5 | 再計画で、同じ人を3回続けて代役にはしない |

割り振れる人がいなければ（全員が辞退・欠席）、確認依頼は出さずに管理者の判断待ちになります。
...
| 担当 | 参加予定で、担当を辞退しておらず、この案件で引き受けを断っていない人 |
