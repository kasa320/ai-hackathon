...
| `POST /api/groups/{group_id}/leave` | メンバー本人 | 200 | **自分が**グループを脱退する。開始前の開催回からは欠席として外れる |
...
`POST /api/groups/{group_id}/leave` は本文に空オブジェクト `{}` を送り、`{"group_id": "...", "affected_session_ids": [...]}` を返す。`affected_session_ids` は本人が外れて再調整が始まった開催回。

- **脱退できるのは本人だけ。** 他人を外すAPIは設けていない。
- **管理者（`owner`）は脱退できない**（`409 invalid_state`）。先に管理者の交代が必要だが、交代APIは未実装のため現状は脱退できない。
- 在籍者が2人を下回る脱退は断る（`409 invalid_state`）。
- **開始前**の開催回からは欠席として外れ、担当辞退と同じ経路で再調整が始まる。未回答の依頼は無効になる。
- **開始済み**の開催回はそのまま残す。誰が担当したかの履歴を保つため、参加対象者の一覧からも消えない。
- 脱退後は当該グループと配下の開催回がすべて `404` になる。ログインし直しても復帰しない。
