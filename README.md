...
開催日時は登録時に入れません。**開始日と終了目安日だけ**を渡すと、参加者から準備状況と「出られない日」を集めたうえで、エージェントが日時と進行をまとめて1つの案にします。管理者の承認と出席予定者の同意がそろった時点で日時が決まります。出られないと答えた人がいる日は、同意の数によらず案になりません。日時を人が決めたいときは、登録時に `starts_at` を渡せば今までどおりです。
